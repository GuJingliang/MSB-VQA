...
def _load_dataset(cache_path, name, img_id2val):
    ...
    return entries
'''
def _load_dataset_sub(cache_path, name, img_id2val):
    """ Load entries. img_id2val: dict {img_id -> val} ,
        val can be used to retrieve image or features.
    """
    train, val, test = False, False, False
    ...
    #counterexamples = json.load(open(os.path.join(config.main_path, 'counterexamples.json'), 'r'))#######vqa-ce
    #hard = json.load(open(os.path.join(config.main_path, 'hard.json'), 'r'))#######vqa-ce
    print(config.subdataset)
    for question, answer in zip(questions, answers):
        
        #########vqa-ce
        #if question['question_id'] in hard or question['question_id'] in counterexamples:
        #    continue
        ##########
        if not test:
            utils.assert_eq(question['question_id'], answer['question_id'])
        ...
        img_id = question['image_id']
        entries.append(_create_entry(img_id2val[img_id], question, answer))
        if len(entries) > 438183 * config.subdataset:
            break
    return entries
'''
def _load_margin(cache_path, name, entries):
    ...
    # qt_dict:Probability of occurrence of different answers in different answer types
    # qt_dict_freq: Number of occurrences
...
class VQAFeatureDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return features, question, target, target_margin, bias, question_id, freq_margin0, q_type, answer_type, index
